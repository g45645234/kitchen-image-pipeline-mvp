...
@pytest.mark.asyncio
async def test_candidate_original_converts_webp_to_inline_jpeg(client, seed_candidate, monkeypatch, tmp_path):
    from app.config import settings
    ...
    file_path = tmp_path / storage_key
    file_path.parent.mkdir(parents=True, exist_ok=True)
    Image.new("RGB", (12, 8), (20, 40, 60)).save(file_path, format="WEBP")
    candidate = await seed_candidate(storage_key_original=storage_key, storage_status="ok")
